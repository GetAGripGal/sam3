...
class Sam3VideoPredictor:
    ...
    def __init__(
        ...
        # Device: CUDA if available, else MPS (Apple Silicon), else CPU (PR 264 / Apple Silicon support)
        # Set SAM3_VIDEO_DEVICE=cpu to force CPU (avoids MPS segfaults on some Macs)
        force_cpu = os.environ.get("SAM3_VIDEO_DEVICE", "").lower() == "cpu"
        if torch.cuda.is_available() and not force_cpu:
            ...
            logger.info(
                "Video predictor using MPS (Apple Silicon). "
                "Performance may differ from CUDA; some ops may fall back to CPU."
            )
        else:
            self.device = torch.device("cpu")
            logger.info("Video predictor using CPU. This may be slow for long videos.")
    ...
    def _get_session_stats(self):
        ...
        if torch.cuda.is_available():
            ...
            mem_str = f"device: {self.device}"
        return f"live sessions: [{', '.join(live_session_strs)}], {mem_str}"
