    ...
    def _get_image_feature(self, inference_state, frame_idx, batch_size):
        ...
        if backbone_out is None:
            if self.backbone is None:
                ...
                # Cache miss -- we will run inference on a single image (use tensor's device for CPU/MPS/CUDA)
                img_t = inference_state["images"][frame_idx]
                image = img_t.float().unsqueeze(0).to(device=img_t.device)
